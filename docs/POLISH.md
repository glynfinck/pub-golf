...
### Wave 1 — in-repo, no consoles, no accounts

All of it is code in this repo and ships in one PR:
...
3. `app/legal/privacy` and `app/legal/terms` + layout, linked from sign-in
   and Profile
4. Responsible-drinking lines (sign-in, rules sheet, recap)
5. Drop `maximumScale`; check input font sizes
6. Manifest fields + maskable icon
7. Metadata fields
8. Security headers; delete the dead `remotePatterns`
9. Resolve the duplicated `error.tsx`
10. Greeting by hour
...
2. Error monitoring
3. Account deletion + the privacy policy paragraph that describes it
4. Version stamp
