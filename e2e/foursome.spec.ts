...
test("a foursome: stampede join, four thumbs on one hole, ties and the substitute", async ({
  ...
  test.setTimeout(budget(150_000));
  ...
  try {
    ...
    // Teardown never decides the verdict, and it now runs even when the
    // body throws: `full-house` already closed in a finally, this spec
    // leaked five contexts into the next test every time it failed.
    await closeQuietly(hostContext, ...guests.map((g) => g.context));
  }
});
